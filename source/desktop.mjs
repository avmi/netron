...
desktop.Host = class {
    ...
    async view(view) {
        ...
        if (process.env.SNAP) {
            ...
            await this.message('Please migrate as Snap support will be discontinued.', null, 'Migrate');
            this.openURL('https://github.com/lutzroeder/netron/issues/1500');
            this.document.body.classList.add('spinner');
        }
        if (process.env.APPIMAGE) {
            this.document.body.classList.remove('spinner');
            await this.message('Please migrate as AppImage support will be discontinued.', null, 'Migrate');
            this.openURL('https://github.com/lutzroeder/netron/issues/1500');
            this.document.body.classList.add('spinner');
        }
        const age = async () => {
